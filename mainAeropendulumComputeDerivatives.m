...
%NL(%) = var(noise)/var(signal)*100
noise_levels = zeros(1,20);
cont=0;
standard_deviation = zeros(1,20);
var_signal = var(theta);
for i=1:20
    noise_levels(1,i)=cont
    cont=cont+1;
    standard_deviation = sqrt(noise_levels*var_signal/100)
end
...
for i = 1:size(theta_noise,2)
        dx(:,i) = TVRegDiff( theta_noise(:,i), 10, 0.00000003, [], 'small', 1e-12, 0.01, 1, 1 ); %.00002
    end
    dx = dx(2:end,:);
